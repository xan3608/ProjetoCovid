...
public class DadosGraficos {
	...
	public static Series<String, Number> seriesCasosAcumulados(List<Dados> dados, int comeco) {
		...
		for (Dados dado : dados) {
			if (indice++ >= comeco) {
				series.getData().add(new XYChart.Data<String, Number>(dado.getDataString(), dado.getCasosAcumulados()));
			}
		}
		return series;
	}
	...
	public static Series<String, Number> seriesCasosNovos(List<Dados> dados, int comeco) {
		...
		for (int i = 1; i < dados.size(); i++) {
			if (i >= comeco) {
				...
				series.getData().add(new Data<String, Number>(dados.get(i).getDataString(), novosCasos));
			}
		}
		return series;
	}
	...
	public static Series<String, Number> seriesObitosAcumulados(List<Dados> dados, int comeco) {
		...
		for (Dados dado : dados) {
			if (indice++ >= comeco) {
				series.getData().add(new XYChart.Data<String, Number>(dado.getDataString(), dado.getObitosAcumulados()));
			}
		}
		return series;
	}
